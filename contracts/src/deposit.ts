...
let config = configJson.deployAliases["gamemanager"];
...
try {
    ...
    const mina = new UInt64(10 * 10 ** 9);
    const tx = await Mina.transaction({ sender: feepayerAddress, fee }, async () => {
        await zkAppGameContract.deposit(mina);
    });
    await tx.prove();
    ...
    if (sentTx.status === 'pending') {
        console.log(
            '\nSuccess! deposit 10 mina transaction sent.\n' +
            '\nYour smart contract will be updated' +
